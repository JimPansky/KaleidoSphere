...
test('distribution builder creates three byte-identical thin host views', async () => {
  ...
  assert.equal(manifest.packageVersion, '0.18.5');
...
test('host manifests stay skills-only and compatible with current schemas', async () => {
  ...
  assert.equal(codexManifest.version, '0.18.5');
  ...
  assert.equal(claudeManifest.version, '0.18.5');
});
...
test('generated artifacts are deterministic and archive-safe', async () => {
  ...
  for (const archive of archives) {
    assert.match(archive.archive, /^archives\/kaleidosphere-(?:clawhub-skill|codex-plugin|claude-plugin)-v0\.18\.5\.tar\.gz$/);
