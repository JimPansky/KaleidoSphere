...
test('release archive checksum is portable from a clean verifier directory', async () => {
  ...
  const checksumPath = path.join(buildDir, 'KaleidoSphere-v0.18.3.tar.gz.sha256');
  const archivePath = path.join(buildDir, 'KaleidoSphere-v0.18.3.tar.gz');
  const checksum = await readFile(checksumPath, 'utf8');
  assert.match(checksum, /^[a-f0-9]{64}  KaleidoSphere-v0\.18\.3\.tar\.gz\n$/);
  ...
  run('sha256sum', ['-c', 'KaleidoSphere-v0.18.3.tar.gz.sha256'], { cwd: verifyDir });

  const listing = run('tar', ['-tzf', 'KaleidoSphere-v0.18.3.tar.gz'], { cwd: verifyDir }).stdout;
  assert(listing.includes('KaleidoSphere-v0.18.3/package.json\n'));
  ...
  for (const name of [
    'kaleidosphere-clawhub-skill-v0.18.3.tar.gz',
    'kaleidosphere-codex-plugin-v0.18.3.tar.gz',
    'kaleidosphere-claude-plugin-v0.18.3.tar.gz',
  ]) {
