...
test('release archive checksum is portable from a clean verifier directory', async () => {
  ...
  const checksumPath = path.join(buildDir, 'KaleidoSphere-v0.11.0.tar.gz.sha256');
  const archivePath = path.join(buildDir, 'KaleidoSphere-v0.11.0.tar.gz');
  const checksum = await readFile(checksumPath, 'utf8');
  assert.match(checksum, /^[a-f0-9]{64}  KaleidoSphere-v0\.11\.0\.tar\.gz\n$/);
  ...
  run('sha256sum', ['-c', 'KaleidoSphere-v0.11.0.tar.gz.sha256'], { cwd: verifyDir });

  const listing = run('tar', ['-tzf', 'KaleidoSphere-v0.11.0.tar.gz'], { cwd: verifyDir }).stdout;
  assert(listing.includes('KaleidoSphere-v0.11.0/package.json\n'));
  assert(!listing.includes('/.git/'));
  assert(!listing.includes('KaleidoSphere-v0.11.0/.env\n'));
