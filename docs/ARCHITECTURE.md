...
`roles-clusters-diff-v2.mjs` consumes only a sealed terminal #37 state and its
receipt-bound #38 evidence. It deterministically projects proposal-only key,
temporal, quality-review and relationship-link roles plus structural connected
components. Counterevidence remains explicit and cannot create a link or merge a
cluster. An engine-neutral semantic digest is nested inside an engine-specific
snapshot; raw values, credentials and automatic business truth remain rejected.

The hash-chained extended diff covers coverage, profiles, relationships,
hypotheses, roles and clusters. Scope and engine must remain identical.
Disappearance is an observed removal only after a successful current query with
complete visibility; denied, unsupported or unknown visibility remains a loss
of visibility, never a deletion claim. Closed restart validators verify nested
projection/diff hashes, bounds, safety flags and non-claims. See
`docs/decisions/ROLES-CLUSTERS-DIFF-V2.md`.

## External API v2

`GET /v2/capabilities` is generated by the running SBA process from its own
package version. It binds product `v0.18.0`, external contract `2.0.0`, exact
